...
const SENSITIVE_SUBJECT_PATTERN = /\b(?:password|passcode|api key|token|secret|address|phone|email|religion|politic|ethnicity|race|sexual|medical|health condition|disability)\b/i;
...
function normalizedSubject(raw) {
    const cleaned = raw
    ...
    if (!cleaned || cleaned.length > 80)
        return undefined;
    if (SENSITIVE_SUBJECT_PATTERN.test(cleaned))
        return undefined;
...
export function inferPreferenceScope(statement) {
    const lower = statement.toLowerCase();
    if (/\b(frontend|front-end|website|web app|ui)\b/.test(lower))
        return "frontend";
    if (/\b(backend|server|api)\b/.test(lower))
        return "backend";
    if (/\b(mobile|android|ios)\b/.test(lower))
        return "mobile";
    if (/\b(desktop|windows|macos|linux app)\b/.test(lower))
        return "desktop";
    return "global";
}
export function extractPreferences(statement, now = new Date().toISOString()) {
    const results = [];
    for (const signal of SIGNALS) {
        ...
        if (!subject)
            continue;
        const category = TECHNOLOGY_ALIASES.has(subject.toLowerCase())
...
export function extractForgottenPreferenceSubjects(statement) {
    if (!/\b(?:forget|remove|delete|clear)\b.*\b(?:preference|favourite|favorite|like|dislike)\b/i.test(statement)) {
        return [];
    }
    const subjects = new Set();
    for (const [alias, canonical] of TECHNOLOGY_ALIASES) {
        if (statement.toLowerCase().includes(alias))
            subjects.add(canonical);
    }
    return [...subjects];
}
export function mergePreference(existing, incoming) {
    if (!existing)
        return incoming;
...
export function rankPreferences(preferences, query = "", scope) {
...
export function recommendFrontendOptions(preferences, task, requestedOptions) {
    ...
    const favorite = rankPreferences(preferences.filter((preference) => preference.category === "technology"
        && preference.sentiment === "like"
        && preference.confidence >= 0.75), "technology", "frontend")[0];
