...
export async function applyVersionedUpdate(version: string): Promise<void> {
  ...
  try {
    ...
    if (!expected || expected !== actual) throw new Error("Release package checksum verification failed");
...
function requestBuffer(url: string, redirects = 0): Promise<Buffer> {
  ...
  return new Promise((resolve, reject) => {
    https.get(
      url,
      { headers: { "User-Agent": "Munch-MCP-Updater" }, timeout: 15_000 },
      (response) => {
        if (
          response.statusCode &&
        ...
        response.on("data", (chunk: Buffer) => {
          size += chunk.length;
          if (size > 25_000_000) response.destroy(new Error("Update asset is too large"));
