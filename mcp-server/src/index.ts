  ...
  async ({ category, symptom, fix, context }) => {
    ...
    if (existing) {
      ...
      if (existing.occurrences >= 3) {
        if (!memory.recurrentMistakes) memory.recurrentMistakes = [];
        let mistake = memory.recurrentMistakes.find(
  ...
  async ({ issue, resolution }) => {
    ...
    if (existing) {
      ...
      if (existing.occurrences >= 3) {
        if (!memory.recurrentMistakes) memory.recurrentMistakes = [];
        let mistake = memory.recurrentMistakes.find(
          (m) => m.symptom.toLowerCase().trim() === cleanIssue || 
...
server.registerTool(
  ...
  {
    description:
      "Create or update a long-horizon task inside the persistent memory timeline. " +
...
server.registerTool(
  ...
  {
    description:
      "Run the Munch BTL validator and hallucination guard on a local file to check for compilation issues and stubs/placeholders.",
    inputSchema: {
