...
export function startHttpServer(options: HttpServerOptions) {
  if (!options.token && !options.allowInsecure) {
    throw new Error(
      "HTTP mode requires MUNCH_HTTP_TOKEN. Set MUNCH_ALLOW_INSECURE_HTTP=true only for isolated local development.",
    );
  }
    ...
    try {
      ...
      if (request.method === "GET" && url.pathname === "/control.json") {
        sendJson(response, 200, {
          ...getControlSnapshot() as Record<string, unknown>,
      ...
      if (!response.headersSent) sendJson(response, 500, { error: "Internal server error", requestId });
    }
  });
  ...
  const shutdown = async () => {
    ...
    for (const entry of sessions.values()) await entry.transport.close();
    for (const transport of legacyTransports.values()) await transport.close();
...
function readJsonBody(request: IncomingMessage, maximum: number): Promise<unknown> {
