...
export type TrustMode = "strict" | "balanced" | "experimental";

interface RuntimeSettings {
  schemaVersion: 1;
  trustMode: TrustMode;
  retentionDays: number;
  sensitivity: Record<string, "public" | "internal" | "sensitive">;
}
...
interface ReferenceCatalog {
  ...
  references: ReferenceCatalogEntry[];
}

interface WorkspaceNode {
  id: string;
  path: string;
  kind: "source" | "test" | "config" | "documentation" | "asset" | "dependency";
  size: number;
}
...
interface WorkspaceEdge {
  from: string;
  to: string;
  type: "imports" | "declares-dependency" | "tests" | "generates";
}
...
function protect(value: unknown): unknown {
  ...
  } satisfies ProtectedValue;
}

function unprotect<T>(value: unknown): T {
  const candidate = value as Partial<ProtectedValue>;
  if (!candidate?.encrypted) return value as T;
  const key = encryptionKey();
  if (!key) throw new Error("MUNCH_RUNTIME_ENCRYPTION_KEY is required to read this artifact");
...
function readProtectedJson<T>(path: string, fallback: T): T {
  if (!existsSync(path)) return fallback;
  return unprotect<T>(JSON.parse(readFileSync(path, "utf8")));
}
...
export function getRuntimeSettings(): RuntimeSettings {
  ensureRuntime();
  const parsed = readJson<RuntimeSettings>(SETTINGS_PATH, defaultSettings());
  return {
    ...defaultSettings(),
    ...parsed,
...
export function configureRuntimeSettings(input: {
  trustMode?: TrustMode;
  retentionDays?: number;
  sensitivity?: RuntimeSettings["sensitivity"];
}): RuntimeSettings {
...
export function compilePolicy(skillPath: string): unknown {
  ...
  const directives = source
    ...
    .filter(({ text }) =>
      /\bMUST\b|\bnever\b|\brequires?_|\bcondition:|\btrigger:|^\s*¬/i.test(text),
    )
    .map((entry) => ({
      ...entry,
...
export function negotiateCapabilities(input: {
  ...
  const contextStrategy =
    input.contextBudget < 8_000 ? "minimal" : input.contextBudget < 32_000 ? "selective" : "expanded";
  return {
    host: input.host,
...
export function detectContradictions(
  statements: Array<{ source: string; text: string; priority?: number }>,
): unknown {
...
export function startTrace(label: string, metadata: Record<string, unknown> = {}): unknown {
  ensureRuntime();
  const id = randomUUID();
  const header = {
    schemaVersion: 1,
...
export function appendTrace(traceId: string, type: string, payload: unknown): TraceEvent {
  const path = join(TRACE_DIR, `${traceId}.jsonl`);
  if (!existsSync(path)) throw new Error(`Trace "${traceId}" does not exist`);
  const lines = readFileSync(path, "utf8").trim().split(/\r?\n/);
  const previous = lines.length > 1 ? JSON.parse(lines.at(-1)!) as TraceEvent : undefined;
  const eventBase = {
  ...
  const event: TraceEvent = { ...eventBase, hash: hash(JSON.stringify(eventBase)) };
  writeFileSync(path, `${JSON.stringify(event)}\n`, { encoding: "utf8", flag: "a" });
  return event;
}

export function replayTrace(traceId: string): unknown {
  const path = join(TRACE_DIR, `${traceId}.jsonl`);
  if (!existsSync(path)) throw new Error(`Trace "${traceId}" does not exist`);
  const lines = readFileSync(path, "utf8").trim().split(/\r?\n/);
...
export function recordProvenance(input: {
  ...
  if (input.confidence < 0 || input.confidence > 1) throw new Error("confidence must be from 0 to 1");
  const graph = readProtectedJson<{ schemaVersion: 1; nodes: any[]; edges: any[] }>(
    PROVENANCE_PATH,
  ...
  graph.edges = graph.edges.filter((edge, index, all) =>
    all.findIndex((candidate) => JSON.stringify(candidate) === JSON.stringify(edge)) === index,
  );
  writeProtectedJson(PROVENANCE_PATH, graph);
  return node;
}

export function getProvenanceGraph(): unknown {
...
export function createEvidenceBundle(input: {
  ...
  requirements: string[];
  changes: string[];
  tests: Array<{ name: string; status: "passed" | "failed" | "skipped"; evidence?: string }>;
  files?: string[];
  risks?: string[];
}): unknown {
  ensureRuntime();
...
export function runEvaluation(input: {
  ...
  cases: Array<{
    ...
    matcher?: "includes" | "exact" | "regex";
  }>;
}): unknown {
  const score = (expected: string, actual: string, matcher = "includes") => {
    if (matcher === "exact") return actual === expected ? 1 : 0;
    if (matcher === "regex") return new RegExp(expected, "i").test(actual) ? 1 : 0;
...
function readCatalog(referenceDir: string): ReferenceCatalog {
  return readJson<ReferenceCatalog>(join(referenceDir, "catalog.json"), { schemaVersion: 1, references: [] });
}

function referenceStats(): Record<string, { uses: number; lastUsed?: string; successes: number; failures: number }> {
  return readJson(REFERENCE_STATS_PATH, {});
}

export function scoreReferences(referenceDir: string, query: string): unknown {
  ...
  const ranked = catalog.references.map((entry) => {
    ...
    const matches = [...queryTokens].filter((token) =>
      `${entry.id} ${entry.title} ${entry.keywords.join(" ")} ${content.slice(0, 6000)}`.toLowerCase().includes(token),
    ).length;
...
export function recordReferenceOutcome(id: string, success: boolean): unknown {
  const stats = referenceStats();
...
export function buildContextPackage(referenceDir: string, query: string, tokenBudget: number): unknown {
  if (!Number.isInteger(tokenBudget) || tokenBudget < 500) throw new Error("tokenBudget must be at least 500");
  const scored = scoreReferences(referenceDir, query) as { ranked: Array<ReferenceCatalogEntry & { score: number; tokenEstimate: number }> };
  const selected: Array<{ id: string; file: string; score: number; content: string; tokenEstimate: number }> = [];
  let used = 0;
  for (const entry of scored.ranked) {
    if (entry.score <= 0 || used + entry.tokenEstimate > tokenBudget) continue;
    const content = readFileSync(join(referenceDir, entry.file), "utf8");
...
export function installReferencePack(sourcePath: string, expectedCoreMajor = 1): unknown {
  const source = resolve(sourcePath);
  const manifestPath = join(source, "munch-reference-pack.json");
  if (!existsSync(manifestPath)) throw new Error("Reference pack manifest is missing");
  const manifest = readJson<{
    id: string;
    version: string;
    compatibleCoreMajor: number;
...
export function listReferencePacks(): unknown {
  ensureRuntime();
...
function classifyPath(path: string): WorkspaceNode["kind"] {
  if (/(^|[\\/])(test|tests|__tests__)([\\/]|$)|\.(test|spec)\./i.test(path)) return "test";
  if (/\.(json|ya?ml|toml|ini|config\.[cm]?[jt]s)$/i.test(path)) return "config";
  if (/\.(md|txt|rst)$/i.test(path)) return "documentation";
  if (/\.(png|jpe?g|gif|svg|ico|woff2?|ttf)$/i.test(path)) return "asset";
  return "source";
}

export function buildWorkspaceGraph(rootPath: string, maximumFiles = 3000): {
  root: string;
  nodes: WorkspaceNode[];
  edges: WorkspaceEdge[];
  truncated: boolean;
  ...
  if (
    process.env.MUNCH_ALLOW_EXTERNAL_WORKSPACE_SCAN !== "true" &&
    root !== workspace &&
  ...
  const byRelative = new Map<string, WorkspaceNode>();
  ...
  while (stack.length && nodes.length < maximumFiles) {
    ...
    for (const entry of readdirSync(directory, { withFileTypes: true })) {
      ...
      const node = { id: `FILE_${hash(path).slice(0, 12)}`, path, kind: classifyPath(path), size: statSync(absolute).size } as WorkspaceNode;
      nodes.push(node);
      byRelative.set(path, node);
      if (nodes.length >= maximumFiles) break;
    }
  }
  for (const node of nodes.filter((entry) => entry.kind === "source" || entry.kind === "test")) {
    ...
    for (const imported of imports) {
      ...
      if (targetNode) edges.push({ from: node.id, to: targetNode.id, type: "imports" });
    }
    if (node.kind === "test") {
...
export function predictChangeImpact(rootPath: string, changedPaths: string[]): unknown {
  ...
  const tests = graph.nodes.filter((node) =>
    node.kind === "test" &&
...
export function purgeExpiredRuntime(now = Date.now()): unknown {
  const settings = getRuntimeSettings();
...
export function getControlSnapshot(): unknown {
  const settings = getRuntimeSettings();
  const provenance = readProtectedJson<{ nodes?: unknown[]; edges?: unknown[] }>(PROVENANCE_PATH, {});
  return {
    status: "online",
    settings,
...
export function renderControlDashboard(snapshot: any): string {
