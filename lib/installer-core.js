...
function linkOwned(source, target, state, dryRun) {
  if (!existsSync(source)) throw new Error(`Required source does not exist: ${source}`);
  ...
  if (!dryRun) {
    if (process.platform === 'win32') {
      ...
      execFileSync('cmd.exe', ['/c', 'mklink', '/J', target, escaped], {
        windowsHide: true,
        stdio: 'pipe',
      });
    } else {
...
export function install(context, {
  ...
  for (const target of plan.skillTargets) {
    if (!codexOnly || target.includes(`${join(context.homeDir, '.agents')}`) || target.includes(`${join(context.homeDir, '.codex')}`)) {
      linkOwned(context.skillDir, target, state, dryRun);
    }
  }
  if (codexOnly) {
...
const REPO_GIT_URL = 'https://github.com/WyvernCW/MunchsPlugin.git';
...
export function setupAutoUpdate(context, { dryRun = false } = {}) {
  ...
  if (context.platform === 'win32') {
    const command = `"${npmExe}" install -g --ignore-scripts --force git+${REPO_GIT_URL}`;
    if (dryRun) return;
    try {
    ...
    if (dryRun) return;
    const cronLine = `0 9 * * * ${npmExe} install -g --ignore-scripts --force git+${REPO_GIT_URL}`;
    try {
