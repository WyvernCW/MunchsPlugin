...
function updateJsonMcpConfig(path, command, args, state, dryRun) {
  ...
  config.mcpServers ??= {};
  config.mcpServers.munch = { command, args, env: {} };
...
function updateCodexConfig(path, command, args, skillPath, marketplacePath, state, dryRun) {
  ...
  content = replaceTomlSection(
    content,
    '[mcp_servers.munch]',
    `command = "${normalize(command)}"\nargs = [${args.map((arg) => `"${normalize(arg)}"`).join(', ')}]`,
  );
...
export function getInstallPlan(context, { includeIfeo = context.platform === 'win32' } = {}) {
  const skillTargets = [
    '.claude/skills/munch',
...
function getIfeoScript(context, action, previousValue) {
  ...
  const debuggerValue = `"${context.nodePath}" "${redirect}"`;
  if (action === 'install') {
    return [
      `$key = '${key}'`,
...
export function install(context, {
  ...
  const plan = getInstallPlan(context, { includeIfeo });
  ...
  for (const target of plan.skillTargets) copyOwned(context.skillDir, target, state, dryRun);
  ...
  if (includeIfeo && context.platform === 'win32') {
    if (!state.registry.ifeoManaged) applyIfeo(context, state, dryRun);
  }
