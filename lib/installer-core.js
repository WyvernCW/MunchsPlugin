...
function installCodexPersonalPlugin(context, state, dryRun) {
  const pluginRoot = join(context.homeDir, 'plugins', 'munch');
  ...
  writeJson(mcpPath, {
    mcpServers: {
      munch: {
        command: normalize(context.nodePath),
        args: [normalize(context.mcpEntry)],
        env: {
          MUNCH_SKILL_PATH: normalize(join(pluginRoot, 'skills', 'munch', 'SKILL.md')),
        },
      },
    },
  }, dryRun);
...
export function getInstallPlan(
  ...
  return {
    codexOnly,
    codexPlugin: {
      root: join(context.homeDir, 'plugins', 'munch'),
