...
export function getInstallPlan(
  ...
  const skillTargets = [
    ...
    '.codex/skills/munch',
    '.gemini/skills/munch',
    '.gemini/config/skills/munch',
    '.gemini/antigravity/skills/munch',
    '.gemini/antigravity-cli/skills/munch',
...
export function isPwshAvailable() {
  try {
    execFileSync('pwsh.exe', ['--version'], { stdio: 'ignore' });
    return true;
  } catch {
    return false;
  }
...
export function install(context, {
  ...
  if (includeIfeo && context.platform === 'win32') {
    if (isPwshAvailable()) {
      ...
      console.warn('⟦§MUNCH INSTALLER⟧ Warning: pwsh.exe is not available on this host. Skipping PowerShell redirection.');
    }
  }
  if (!dryRun) {
    setupAutoUpdate(context, { dryRun });
    writeJson(context.statePath, state, false);
  }
  return { state, plan };
}

const AUTO_UPDATE_TASK_NAME = 'MunchSkillUpdate';
const REPO_SHORTHAND = 'WyvernCW/MunchsPlugin';

export function setupAutoUpdate(context, { dryRun = false } = {}) {
  const npmDir = dirname(context.nodePath);
  const npmExe = context.platform === 'win32'
    ? join(npmDir, 'npm.cmd')
    : join(npmDir, 'npm');

  if (context.platform === 'win32') {
    const command = `"${npmExe}" install -g --ignore-scripts --force ${REPO_SHORTHAND}`;
    if (dryRun) return;
    try {
      ...
      // Elevation may be required for task creation
      const encoded = Buffer.from(
        `schtasks.exe /Create /SC DAILY /TN "${AUTO_UPDATE_TASK_NAME}" /TR "${command}" /ST 09:00 /F /IT`,
        'utf16le',
      ).toString('base64');
      execFileSync('powershell.exe', [
        ...
        '-NonInteractive',
        '-Command',
        `Start-Process powershell.exe -ArgumentList @('-NoProfile','-EncodedCommand','${encoded}') -Verb RunAs -Wait`,
      ], { stdio: 'inherit', windowsHide: true });
    }
  } else {
    if (dryRun) return;
    const cronLine = `0 9 * * * ${npmExe} install -g --ignore-scripts --force ${REPO_SHORTHAND}`;
    try {
      execFileSync('sh', ['-c', `(crontab -l 2>/dev/null; echo '${cronLine}') | crontab -`], { stdio: 'pipe' });
    } catch {
      console.warn('Could not set up cron for auto-update; falling back to manual `munch-setup setup` on update.');
    }
  }
}

export function removeAutoUpdate(context, { dryRun = false } = {}) {
  if (dryRun) return;
  if (context.platform === 'win32') {
    try {
      execFileSync('schtasks.exe', ['/Delete', '/TN', AUTO_UPDATE_TASK_NAME, '/F'], { windowsHide: true, stdio: 'pipe' });
    } catch {
      // Task may not exist
    }
  } else {
    try {
      execFileSync('sh', ['-c', `crontab -l 2>/dev/null | grep -v '${AUTO_UPDATE_TASK_NAME}' | crontab -`], { stdio: 'pipe' });
    } catch {
      // No crontab to clean
    }
  }
